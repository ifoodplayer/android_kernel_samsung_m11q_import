...
#define AFE_PORT_ID_AWDSP_TX			(AFE_PORT_ID_QUATERNARY_MI2S_TX)
...
static int32_t afe_callback(struct apr_client_data *data, void *priv)
{
	...
	if (data->opcode == AFE_PORT_CMDRSP_GET_PARAM_V2) {
		...
		if (rtac_make_afe_callback(data->payload,
			...
			/* HS70-24 bring up aw smartpa by xuyuqing at 2019/10/31 end */
			#ifdef CONFIG_TAS25XX_ALGO
			if (atomic_read(&this_afe.tas_state) == 1)
			{
 				u32 *payload32 = data->payload;
				pr_err ("[TI-SmartPA:%s] payload1 = 0x%x, 0x%x", __func__, payload32[0], payload32[1]);
		...
		if (data->opcode == APR_BASIC_RSP_RESULT) {
			...
			case AFE_PORT_CMD_SET_PARAM_V2:
				...
				if (atomic_read(&this_afe.tas_state) == 1) {
					atomic_set(&this_afe.status, payload[1]);
					atomic_set(&this_afe.tas_state, 0);
				}
				#endif
...
/* HS60 add HS60-29 integrate nxp smartpa cal func by xudayi at 2019/07/23 start */
/* HS70-24 bring up aw smartpa by xuyuqing at 2019/10/31 start */
#if defined(CONFIG_SND_SOC_TFA98XX) || defined(CONFIG_SND_SMARTPA_AW882XX)
...
int aw_send_afe_cal_apr(uint32_t param_id, void *buf, int cmd_size, bool write)
{
	int32_t result = 0, port_id = AFE_PORT_ID_QUINARY_MI2S_RX;
...
{
	int32_t port_id = AFE_PORT_ID_QUINARY_MI2S_RX;
	uint32_t port_index = 0, opcode;
	uint32_t apr_msg_size = 0;
	uint32_t apr_msg[48];
	size_t len;
...
{
	union afe_spkr_prot_config afe_spk_config;
	int32_t port_id = AFE_PORT_ID_QUINARY_MI2S_TX;
...
static int32_t tas_smartamp_algo_callback(uint32_t *payload, uint32_t payload_size)
{
	...
	pr_debug("[TI-SmartPA:%s] callback, %d", __func__, resp->pdata.param_id);
	...
	if (!this_afe.tas_calib_data.status) {
		...
		atomic_set(&this_afe.tas_state, -1);
	}
		...
		int32_t length, uint8_t *data)
{
	...
	ret = wait_event_timeout(this_afe.wait[index],
			(atomic_read(&this_afe.tas_state) == 0),
