...
static const struct snd_soc_dapm_route audio_map[] = {
	...
	/* HS60 add HS60-304 solve sound result current abnormal by xudayi at 2019/08/01 start */
	{"Ext Spk Switch", "On", "LINE_OUT"},
	/* HS60 add HS60-304 solve sound result current abnormal by xudayi at 2019/08/01 end */
...
/* huaqin diff speaker dapm route by xuyuqing at 2019/10/22 start */
static const struct snd_soc_dapm_route audio_map_hph[] = {
	/* RDAC Connections */
	{"HPHR DAC", NULL, "RDAC2 MUX"},
	{"RDAC2 MUX", "RX1", "PDM_IN_RX1"},
	{"RDAC2 MUX", "RX2", "PDM_IN_RX2"},
...
/* huaqin diff speaker dapm route by xuyuqing at 2019/10/22 end */
						...
						int event)
{
	...
	case SND_SOC_DAPM_PRE_PMD:
		...
		/* HS60 modify HS60-415 slove speaker pop sound by xudayi at 2019/08/12 start */
		msleep(10);
		/* HS60 modify HS60-415 slove speaker pop sound by xudayi at 2019/08/12 end */
		break;
	}
	return 0;
}
...
static int msm_anlg_cdc_soc_probe(struct snd_soc_codec *codec)
{
	...
	/* HS60 modify ZQL1693-53 Codec MIC_IN3_N internal pull-up changed to ground by lanshiming at 2019/10/1 start */
	snd_soc_update_bits(codec, MSM89XX_PMIC_ANALOG_MICB_1_INT_RBIAS,0x01, 0x00);
	pr_err("%sXXXtest:set bit 0 of reg 0x143 to 0 \n",__func__);
	/* HS60 modify ZQL1693-53 Codec MIC_IN3_N internal pull-up changed to ground by lanshiming at 2019/10/1 end */
...
/* huaqin diff speaker dapm route by xuyuqing at 2019/10/22 start */
static struct snd_soc_codec_driver soc_codec_dev_sdm660_cdc_hph = {
	.probe	= msm_anlg_cdc_soc_probe,
	.remove	= msm_anlg_cdc_soc_remove,
	.suspend = msm_anlg_cdc_suspend,
	.resume = msm_anlg_cdc_resume,
	...
	.component_driver = {
		...
		.dapm_widgets = msm_anlg_cdc_dapm_widgets,
		.num_dapm_widgets = ARRAY_SIZE(msm_anlg_cdc_dapm_widgets),
		.dapm_routes = audio_map_hph,
		.num_dapm_routes = ARRAY_SIZE(audio_map_hph),
	},
};
/* huaqin diff speaker dapm route by xuyuqing at 2019/10/22 end */
...
static int msm_anlg_cdc_probe(struct platform_device *pdev)
{
	...
	int adsp_state;
	/* huaqin diff speaker dapm route by xuyuqing at 2019/10/22 start */
	const char *distinguish_speaker = "qcom,distinguish_speaker";
	const char *speaker_flag = NULL;
	/* huaqin diff speaker dapm route by xuyuqing at 2019/10/22 end */
	...
	ret = of_property_read_string(pdev->dev.of_node,
		distinguish_speaker, &speaker_flag);
	if (ret) {
		dev_err(&pdev->dev, "%s: missing %s in dt node\n",
			__func__, distinguish_speaker);
		pr_err("distinguish_speaker_no_find_node\n");
	}
	if (speaker_flag == NULL) {
		pr_err("set to old speaker dapm driver\n");
		ret = snd_soc_register_codec(&pdev->dev,
				     &soc_codec_dev_sdm660_cdc,
				     msm_anlg_cdc_i2s_dai,
				     ARRAY_SIZE(msm_anlg_cdc_i2s_dai));
	} else if (!strcmp(speaker_flag, "CDC_HPH")) {
		pr_err("set to new speaker dapm driver\n");
		ret = snd_soc_register_codec(&pdev->dev,
				     &soc_codec_dev_sdm660_cdc_hph,
				     msm_anlg_cdc_i2s_dai,
				     ARRAY_SIZE(msm_anlg_cdc_i2s_dai));
	}
	/* huaqin diff speaker dapm route by xuyuqing at 2019/10/22 end */
